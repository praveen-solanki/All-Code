...
class videoGUI:
    ...
    def __init__(self, window, window_title):
        ...
        self.canvas.pack()
        self.myimage = cv2.resize(cv2.imread('start.jpeg'),(380,300))
        self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(cv2.cvtColor(self.myimage, cv2.COLOR_BGR2RGB)))   # convert BGR into RGB color map
        self.canvas.create_image(0, 0, image = self.photo, anchor = NW)

        # Select Button
        self.btn_select=Button(bottom_frame, text="Select video file", bg='#80CBC4', width=15, command=self.open_file)
        self.btn_select.grid(row=0, column=0)

         # cam Button
        self.btn_Back=Button(bottom_frame, text="Live Cam", width=15, bg='#80CBC4',command=self.open_cam)
        self.btn_Back.grid(row=2, column=0)

        # recognition Button
        self.btn_quit=Button(bottom_frame, text="Recognition", width=15, bg='#80CBC4')
        self.btn_quit.grid(row=2, column=2)

        # Play Button
        self.btn_play=Button(bottom_frame, text="Play", width=15, state=DISABLED, bg='#80CBC4', command=self.play_video)
        self.btn_play.grid(row=0, column=1)

        # Pause Button
        self.btn_pause=Button(bottom_frame, text="Pause", width=15, state=DISABLED, bg='#80CBC4', command=self.pause_video)
        self.btn_pause.grid(row=0, column=2)

        # Face Detection Label
        self.lbl_face_detection = Button(bottom_frame, text="Face Detection :", width=15, bg='#80CBC4',)
        self.lbl_face_detection.grid(row=1, column=0)

        # Face Detection Record
        self.btn_record = Button(bottom_frame, text="Record", width=15, state=DISABLED, bg='#80CBC4', command=self.start_record)
        self.btn_record.grid(row=1, column=1)

        # Face Detection Stop Recording
        self.btn_stop = Button(bottom_frame, text="Stop recording", width=15, state=DISABLED, bg='#80CBC4', command=self.stop_record)
        self.btn_stop.grid(row=1, column=2)

        # Snapshot Button
        self.btn_snapshot=Button(bottom_frame, text="Snapshot", width=15, state=DISABLED, bg='#80CBC4', command=self.snapshot)
    ...
    def open_file(self):
        self.pause = False
        self.status['text'] = 'play opened file'
        ...
        if self.width >=1520 and self.width >= 660:
            cv2.resize(vid,(1420,580))
            self.width = 1520
            self.height = 620
        #self.width = self.width/1.5
        #self.height = self.height/1.5
        self.canvas.config(width = self.width, height = self.height)
        self.myimage = cv2.resize(cv2.imread('taptoplay.jpg'),(self.width,self.height))
        self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(cv2.cvtColor(self.myimage, cv2.COLOR_BGR2RGB)))   # convert BGR into RGB color map
        self.canvas.create_image(0, 0, image = self.photo, anchor = NW)

    def open_cam(self):
        self.myimage = cv2.resize(cv2.imread('play.png'),(640,480))
        self.pause = False
        self.status['text'] = 'cam opened'
    ...
    def face_detection(self):
        self.pause = False
        self.status['text'] = 'Detecting face'
        self.btn_play['state'] = NORMAL
        ...
        #self.cap = cv2.VideoCapture(0)
        #self.face_cascade ='C:\\Users\\praveen solanki\\Desktop\\Open-CV\\cascades\\haarcascade_frontalface_default.xml'
    	...
    	#    frame = cv2.rectangle(cv2.cvtColor(frame,cv2.COLOR_RGB2BGR), (x, y), (x+w, y+h), (173,120,45), 2)
        #print(x,y,w,h)
    ...
    def play_video(self):
        ...
        self.status['text'] = 'playing video'
        self.btn_pause['state'] = NORMAL
        self.btn_snapshot['state'] = NORMAL
        self.btn_record['state'] = NORMAL
        self.btn_stop['state'] = NORMAL
    ...
    def snapshot(self):
        self.status['text'] = 'Screenshot captured'
        # Get a frame from the video source
        ret, frame = self.get_frame()
        save_location_img = 'C:\\Users\\praveen solanki\\Desktop\\face recignition\\frame_images\\'
        if ret:
            cv2.imwrite(save_location_img + "frame-" + time.strftime("%d-%m-%Y-%H-%M-%S") + ".png", frame)
    ...
    def rec(self,frame):
        ...
        for (x, y, w, h) in faces:
            ...
            #             x:x + w]
            #face_frame = cv2.resize(face_frame, self.result_frame_size)
            face_frame = cv2.rectangle(frame, (x, y), (x+w, y+h), (173,120,45), 2)
            self.photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(cv2.cvtColor(face_frame, cv2.COLOR_RGB2BGR)))   # convert BGR into RGB color map
            self.canvas.create_image(0, 0, image = self.photo, anchor = NW)
    ...
    def stop_record(self):
        ...
        self.status['text'] = 'Recording stopped'
